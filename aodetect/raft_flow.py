...
class RAFTFlowDetector:
    ...
    def compute_raft_flow(self, frame1, frame2):
        """Compute optical flow using RAFT (if available)"""
        if not PYTORCH_AVAILABLE or not RAFT_AVAILABLE or self.use_opencv_fallback:
            return self.compute_opencv_flow(frame1, frame2)
        
        try:
            # Convert frames to tensors
            with torch.no_grad():
                # Resize frames for RAFT processing
                h, w = frame1.shape
                frame1_resized = cv2.resize(frame1, (640, 480))
                frame2_resized = cv2.resize(frame2, (640, 480))
                
                # Convert to RGB (RAFT expects RGB)
                frame1_rgb = cv2.cvtColor(cv2.cvtColor(frame1_resized, cv2.COLOR_GRAY2BGR), cv2.COLOR_BGR2RGB)
                frame2_rgb = cv2.cvtColor(cv2.cvtColor(frame2_resized, cv2.COLOR_GRAY2BGR), cv2.COLOR_BGR2RGB)
                
                # Convert to tensors
                img1 = self.transform(frame1_rgb).unsqueeze(0).to(self.device)
                img2 = self.transform(frame2_rgb).unsqueeze(0).to(self.device)
                
                # For now, use a simple dense optical flow approximation
                # In a full RAFT implementation, you would load the pretrained model here
                flow = self.compute_dense_flow_approximation(img1, img2)
                
                # Resize flow back to original dimensions
                flow_resized = F.interpolate(flow, size=(h, w), mode='bilinear', align_corners=False)
                flow_np = flow_resized.squeeze(0).cpu().numpy()
                
                return flow_np
                
        except Exception as e:
            print(f"RAFT flow computation failed: {e}, falling back to OpenCV")
            return self.compute_opencv_flow(frame1, frame2)
    
    def compute_dense_flow_approximation(self, img1, img2):
        """
        Approximation of dense optical flow using PyTorch operations
        This is a simplified version - full RAFT would use the pretrained model
        """
        ...
        # Compute gradients
        grad_x = torch.gradient(img1, dim=3)[0]
        grad_y = torch.gradient(img1, dim=2)[0]
        grad_t = img2 - img1
        
        # Lucas-Kanade approximation in PyTorch
        fx = grad_x.mean(dim=1, keepdim=True)
        fy = grad_y.mean(dim=1, keepdim=True)
        ft = grad_t.mean(dim=1, keepdim=True)
        
        # Solve optical flow equation (simplified)
        denominator = fx * fx + fy * fy + 1e-8
        u = -(fx * ft) / denominator
        v = -(fy * ft) / denominator
    ...
    def process_frame_pair(self, current_frame, min_area=5, max_area=500):
        ...
        try:
            flow = self.compute_raft_flow(frame1, frame2)
            if flow is None:
                return []
